...
function QuantityBox({ item, outOfStock, decreaseCount, increaseCount }) {
  return (
    <div className="aq-product-quantity">
      ...
      <button
        ...
        disabled={outOfStock}
        onClick={() =>
          increaseCount(item?.productId, item?.quantity, item?.totalQuantity)
        }
...
const CartProducts = ({
  ...
  return (
    <>
      <div className="aq-cart-list aq-cart-list-desktop">
      ...
      <div className="aq-cart-mobile-list">
        {products?.map((item) => {
          const outOfStock = parseFloat(item?.totalQuantity) <= 0;
          const href = `/product-detail?id=${item?.productId}`;
          ...
          return (
            <div
              key={`m-${item.id}`}
              className={`aq-cart-mobile-item${outOfStock ? " is-oos" : ""}`}
            >
              <Link href={href} className="aq-cart-mobile-thumb">
                <img src={item?.imgSrc || "/images/home/KCLogo.png"} alt={item.title || "Product"} />
                {outOfStock ? <span className="aq-cart-oos">Out of Stock</span> : null}
              </Link>
              <div className="aq-cart-mobile-summary">
                <Link href={href} className="aq-cart-title">
                  {item?.title}
                </Link>
                <span className="aq-cart-mobile-price">Rs. {item?.price}</span>
                <div className="aq-cart-mobile-actions">
                  <QuantityBox
                    item={item}
                    outOfStock={outOfStock}
                    decreaseCount={decreaseCount}
                  ...
                  <button
                    type="button"
                    className="aq-cart-mobile-remove"
                    onClick={(event) => removeFromCart(event, item?.id)}
                  >
                    Remove
                  </button>
