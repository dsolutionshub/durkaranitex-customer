    ...
    return (
        <div className="flex flex-wrap justify-between">
            {categories.map((cat, index) => (
                <div
                    key={index}
                    className={`relative rounded-lg overflow-hidden shadow-lg ${cat.isTop ? 'w-full h-96 mb-10' : 'w-full sm:w-[90%] md:w-[48%] lg:w-[31%] h-96'
                        }`}
                >
                    <Image
                        src={cat.image}
...
export default function CategorySection() {
    return (
        <Section title={"Explore Categories"} desc={"Find the perfect saree for every occasion"} section={<Categorycard />} />
    );
}
